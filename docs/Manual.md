...
For more details, consult the [XO-Chip specification](XO-ChipSpecification.md) in Octo's documentation directory. Octo is the main Chip-8 interpreter which supports these instructions, but a small but growing collection of third-party interpreters support them as well. Why not add them to yours?
